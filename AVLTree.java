     ...
     private Node deleteElement(int value, Node node) {
        ...
        if (value < node.value) {
            ...
            if (node.leftChild == null && node.rightChild == null) {
                return null;
            } // case of 0 children, return null
            ...
            } // case of 1 child assign subtree to current node
            if (node.rightChild != null && node.leftChild != null) {
            ...
            } // case of 2 children, find in-order successor, assign value to current node
              // delete in-order-successor
        }

        node.height = (getMaxHeight( getHeight(node.leftChild), getHeight(node.rightChild))) + 1; // update node height
        int bf = getBalanceFactor(node); // calculate balance factor

        if (bf > 1) { // right heavy
            if (getBalanceFactor(node.leftChild) >= 0) {
                node = LLRotation(node); // child right heavy, left left
            } else if (getBalanceFactor(node.leftChild) < 0) {
                node = LRRotation(node); // child left heavy, left right
            } 
        } 
        else if (bf < -1) { // left heavy
            if (getBalanceFactor(node.rightChild) > 0) {
                node = RLRotation(node); // child right heavy, right left
            } else if (getBalanceFactor(node.rightChild) <= 0) {
                node = RRRotation(node); // child left heavy, right right
            } 
        } // left heavy parent
